...
export default function DashboardPage() {
  ...
  return <>
    ...
    <section className="mt-5 grid gap-5 lg:grid-cols-3">
      ...
      {ordersEnabled ? <Card><CardHeader className="flex-row items-start justify-between gap-3"><div><CardTitle>Recent orders</CardTitle><p className="mt-1 text-sm text-muted-foreground">Latest customer order requests.</p></div><CardLink href={modulePrimaryPath("orders")}>All orders</CardLink></CardHeader><CardContent>{loading && !orders ? <Skeleton className="h-56 w-full" /> : orders?.records.length ? <ul className="divide-y">{orders.records.slice(0, 5).map((record) => <li key={record.id} className="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0"><div className="min-w-0"><p className="truncate text-sm font-medium">{record.title}</p><p className="mt-1 text-xs text-muted-foreground">{relativeTime(record.created_at)}</p></div><Badge variant={statusVariant(recordStatus(record))}>{words(recordStatus(record))}</Badge></li>)}</ul> : <Empty>No orders yet.</Empty>}</CardContent></Card> : null}
