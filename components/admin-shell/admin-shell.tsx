...
function NavItem({ item, active, collapsed, onNavigate }: { item: NavigationLink; active: boolean; collapsed: boolean; onNavigate?: () => void }) {
  ...
  const link = <Link href={item.href} onClick={onNavigate} aria-current={active ? "page" : undefined} className={cn("admin-nav-link flex h-9 items-center gap-3 rounded-md px-2.5 text-sm font-medium", active ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-accent hover:text-foreground", collapsed && "justify-center px-0")}><Icon className="size-4 shrink-0" /><span className={cn("truncate", collapsed && "sr-only")}>{item.label}</span></Link>;
...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  const sidebar = useMemo(() => {
    ...
    if (settingsItems.length) {
      ...
      if (settingsGroup) settingsGroup.items.unshift(...settingsItems);
      else groups.push({ label: "Settings", items: settingsItems });
    }
  ...
  return <div className="admin-shell-grid min-h-screen bg-background lg:grid" style={{ gridTemplateColumns: sidebarRight ? `minmax(0,1fr) ${desktopWidth}` : `${desktopWidth} minmax(0,1fr)` }}>
    <aside className={cn("admin-sidebar hidden h-screen flex-col border-r lg:sticky lg:top-0 lg:flex", sidebarSoft ? "bg-accent/35" : "bg-card", sidebarRight && "order-2 border-l border-r-0")}><SidebarContent {...sidebarProps} collapsed={collapsed} onCollapse={toggleCollapsed}>{nav(collapsed)}</SidebarContent></aside>
